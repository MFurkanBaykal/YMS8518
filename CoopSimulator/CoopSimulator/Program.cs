...
namespace CoopSimulator
{
    class Program
    {
        ...
        {
            Console.WriteLine(Program.Date.Date.ToString("dd-MM-yyyy") + "| F: " + Program.PoultryList.Count(a => a.Sex == Enums.PoultrySex.Female) + "| M: " + Program.PoultryList.Count(a => a.Sex == Enums.PoultrySex.Male) + "| AVG: " + (int)Program.PoultryList.Average(a => (Program.Date.Date - a.BirthDate).TotalDays));
        }
    }
}
